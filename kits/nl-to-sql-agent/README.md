...
The command loads TypeScript modules through `tsx`, so it works on Node 20 and later. Individual suites can also be run directly — from the kit root (`kits/nl-to-sql-agent/`) with `node tests/<name>.test.js`, or from `apps/` with `node ../tests/<name>.test.js` (Node 24 type-strips the `.ts` imports on its own).
